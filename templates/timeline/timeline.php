...
$ev_post_img = esc_url(ect_get_event_image($event_id, $size = 'large'));
if ($i % 2 == 0) {
    $even_odd = 'even';
} else {
    $even_odd = 'odd';
...
if ($timeline_style == 'style-1') {
    ...
    $events_html .= '<div id="post-' . esc_attr($event_id) . '" class="ect-timeline-post ' . esc_attr($even_odd) . ' ' . esc_attr($event_type) . ' ' . esc_attr($timeline_style) . ' ' . esc_attr($time) . '">';
